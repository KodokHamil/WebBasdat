...
<?php 
if(isset($_SESSION['admin_email']) == ''){ //belum dalam keadaan login
...
if(isset($_POST['simpan'])){
    ...
    if($password != ''){ //jika akan melakukan perubahan password
        $sql1 = "select * from admin where email = '".$_SESSION['admin_email']."'";
    ...
    if(empty($err)){
        $sql1 = "update admin set nama_lengkap = '".$nama_lengkap."' where email = '".$_SESSION['admin_email']."'";
        mysqli_query($koneksi,$sql1);
        $_SESSION['admin_nama_lengkap'] = $nama_lengkap;

        if($password){
            $sql2 = "update admin set password = md5($password) where email = '".$_SESSION['admin_email']."'";
...
<form action="" method="POST">
    <table>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="nama_lengkap" class="input" value="<?php echo $_SESSION['admin_nama_lengkap']?>"/>
            </td>
        </tr>
        <tr>
